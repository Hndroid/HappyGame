...
import com.happiest.game.app.shared.dialog.MessageDialog
...
abstract class PermissionCallback : OnPermissionCallback {
    ...
    override fun onDenied(permissions: MutableList<String>, never: Boolean) {
        ...
        val activity: Activity? = ActivityStackManager.getInstance().getTopActivity()
        if (permissions.size == 1 && (Permission.ACCESS_BACKGROUND_LOCATION == permissions[0])) {
            Toast.makeText(activity?.baseContext, R.string.common_permission_fail_4, Toast.LENGTH_LONG).show()
            return
        }
        Toast.makeText(activity?.baseContext, R.string.common_permission_fail_1, Toast.LENGTH_LONG).show()
    }
