 ...
 */
class SimpleRatingBar @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    fun setGrade(grade: Float) {
        var grade: Float = grade
        if (grade > gradeCount) {
            grade = gradeCount.toFloat()
        }
        if (grade - grade.toInt() != 0.5f || grade - grade.toInt() > 0) {
