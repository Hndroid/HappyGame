 ...
 *    desc   : 高仿 ios 开关按钮
 */
class SwitchButton @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0, defStyleRes: Int = 0) :
    ...
    private fun calcBTranslate(percent: Float): Float {
        ...
        when (checkedState - lastCheckedState) {
            1 -> if (checkedState == STATE_SWITCH_OFF2) {
                // off -> off2
                result = offLeftX
            ...
            2 -> if (checkedState == STATE_SWITCH_ON) {
                // off2 -> on
            ...
            3 ->                 // off -> on
                result = onLeftX - (onLeftX - offLeftX) * percent
            -1 -> if (checkedState == STATE_SWITCH_ON2) {
                // on -> on2
            ...
            -2 -> if (checkedState == STATE_SWITCH_OFF) {
                // on2 -> off
            ...
            0 -> if (checkedState == STATE_SWITCH_OFF) {
                //  off -> off
                result = offLeftX
            } else if (checkedState == STATE_SWITCH_ON) {
                // on -> on
